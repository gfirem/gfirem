...
jQuery(document).ready(function ($) {
	...
	function load_fields() {
		$('.gfirem_select_image').each(function () {
			var is_ready = $(this).attr('data-ready');
			...
			var element_key = id;
			if (gfirem_select_image.config[element_key] === undefined) {
				element_key = $('.gfirem_select_image[data-ready="true"]').attr('field_id');
			}
			if (gfirem_select_image.config[element_key].image_url) {
				old_full_size = gfirem_select_image.config[element_key].image_url;
			}
			if (gfirem_select_image.config[element_key].activate_zoom && gfirem_select_image.config[element_key].activate_zoom === 'true') {
				if (gfirem_select_image.config[element_key].scroll_zoom && gfirem_select_image.config[element_key].scroll_zoom === 'true') {
			...
			$('input[name="' + id + '"][type="button"]').click(function (e) {
				...
				mediaUploader = wp.media.frames.file_frame = wp.media({
					title: gfirem_select_image.config[element_key].library_title,
					button: {
						text: gfirem_select_image.config[element_key].library_button_title
					}, multiple: false
				});
